...
namespace Back_Project.code.Tool.Reader
{
    class CellNodeReader
    {
        ...
        {
            //先判断有没有Font节点
            if (_cellElement.GetElementsByTagName("Font").Count > 0)
            {
                if (isKeyNode)
                {
                    Console.WriteLine("存在Font节点，错误的节点是" + _sheetName + "表格的key节点");
                } 
                else
                {
                    Console.WriteLine("存在Font节点，错误的节点是" + _sheetName + "-" + _key + "-" + (_rowIndex + 1) + "列");
                }
                return null;
            }
            int dataType = -1;
